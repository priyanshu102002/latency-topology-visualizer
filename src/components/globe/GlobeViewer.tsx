...
const getNodeColor = (node: any) =>
  PROVIDER_COLORS[node.provider as CloudProvider] || "#ffffff";
const getNodeAltitude = (node: any) =>
  node.type === NodeType.EXCHANGE ? 0.12 : 0.05;
...
const GlobeViewer: React.FC<GlobeViewerProps> = React.memo(
  ({ nodes, links, onNodeClick, width, height }) => {
    ...
    const ringsData = useMemo(
      () => nodes.filter((n) => n.type === NodeType.EXCHANGE),
      [nodes]
    );

    const handlePointClick = useCallback(
      (point: object) => onNodeClick(point as GeoNode),
      [onNodeClick]
    );
    const handlePointHover = useCallback(
      (point: object | null) => setHoveredNode(point as GeoNode | null),
      []
    );
    ...
    return (
      <div className="relative">
        ...
        {hoveredNode && (
          ...
          >
            <p className="font-bold text-emerald-600 dark:text-emerald-400 text-sm mb-1">
              {hoveredNode.name}
            </p>
